...
public class CurrentReleaseData {
	...
	public void getCurrentVersions(String type) {
		try {
			...
			InputStream incomingVersionData = httpCall
				.makeCall(versionType, whatVersion.formNeo4jVersionRequest(versionType));

			// recieve the version data and parse it
			versions = parser.parseVersion(incomingVersionData, versionType);
	...
	public void getRawLocusData() {
		try{
			// retrieve the data
	        // create the request and send it
	        Neo4jRequests request = new Neo4jRequests();
	        InputStream incomingData = httpCall.makeCall(versionType, 
			...
			switch (versionType) {
				case "HLA":
					...
					break;
				default:
					System.out.println(versionType + " hasn't been added yet.");
			}
